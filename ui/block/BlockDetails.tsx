...
const getExtraData = async (hash: string): Promise<ValidatorAddress[]> => {
  try {
    if (!jsonRpcHost) {
      // console.error('JSON-RPC host is not defined');
      return [];
    }
    ...
    const data = await response.json() as { result?: { hash?: string, extraData?: string } };

    if (!data.result?.extraData || !data.result?.hash) {
      return [];
    }

    const extraData = data.result.extraData.startsWith('0x') ?
      data.result.extraData.slice(2) :
      data.result.extraData;

    const validators: ValidatorAddress[] = [];
    const committer = new Set<string>();
    try {

      /*
       extraData 구조:
       array[0] = vanity, array
       array[1] = validator address, array
       array[2] = vote
       array[3] = round
       array[4] = committedSeal, array
      */
      const extraDataBytes = ethers.getBytes('0x' + extraData);

      //@ts-ignore
      const decodedExtraData = rlp.decode(extraDataBytes) as any[];
      ...
      const validatorAddresses = decodedExtraData[1] as Uint8Array[];
      const committerSignatures = decodedExtraData[4] as Uint8Array[];

      //console.log("block hash", data.result?.hash);

      // 서명 복구
      if (Array.isArray(committerSignatures)) {
        for (let i = 0; i < committerSignatures.length; i++) {
          const committerSignature = committerSignatures[i];

          try {
            const messageHash = data.result.hash;

            // 서명 데이터 처리
            const signatureHex = ethers.hexlify(committerSignature);
            ...
            // console.log('committerSignature', signatureHex.slice(2));

            if (recoveredAddress) {
              committer.add(recoveredAddress.toLowerCase());
            }
          } catch (error) {
      ...
      if (Array.isArray(validatorAddresses)) {
        for (let i = 0; i < validatorAddresses.length; i++) {
          ...
          const address = ethers.hexlify(validatorAddress);

          // console.log('address', ethers.hexlify(validatorAddress));

          validators.push({
            address: address,
            committer: committer.has(address)
          });
        }
      }
    ...
    // console.error('Error fetching block extra data:', error);
    return [];
  }
};
...
const BlockDetails = ({ query }: Props) => {
  ...
  const [Validators, setValidators] = React.useState<ValidatorAddress[]>([]);
  ...
  return (
    ...
    >
      ...
      {Validators && (
        <>
          <DetailsInfoItem.Label
            hint="Extra Data includes validators and committers"
            isLoading={isPlaceholderData}
          >
            Extra Data
          </DetailsInfoItem.Label>
          <DetailsInfoItem.Value>
            <Skeleton isLoaded={!isPlaceholderData}>
              <select multiple aria-readonly>
                {Validators.map((v) => {
                  return <option key={v.address}> <span style={{ fontWeight: v.committer ? 'bold' : 'normal' }}>{v.address} {v.committer ? '(committer)' : ''}</span></option>
                })}
              </select>
            </Skeleton>
          </DetailsInfoItem.Value>
