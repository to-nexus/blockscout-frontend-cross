...
const getFeePayer = async (hash: string, txType: number | null): Promise<FeePayer | null> => {
  if (txType !== 7) {
    return null;
  }
  try {
    ...
    const data = await response.json() as { result?: FeePayer };
...
const TxInfo = ({ data, isLoading, socketStatus }: Props) => {
  const [feePayer, setFeePayer] = React.useState<string | null>(null);

  React.useEffect(() => {
    if (data?.hash) {
      getFeePayer(data.hash, data.type).then(result => {
        setFeePayer(result?.feePayer ?? null);
      });
    }
  }, [data?.hash]);
  ...
  return (
    <Grid columnGap={8} rowGap={{ base: 3, lg: 3 }} templateColumns={{ base: 'minmax(0, 1fr)', lg: 'max-content minmax(728px, auto)' }}>
      ...
      {feePayer && (
        <>
          <DetailsInfoItem.Label
            hint="Address that paid the fee"
            isLoading={isLoading}
          >
            Fee Payer
          </DetailsInfoItem.Label>
          ...
          >
            <Flex flexWrap="nowrap" alignItems="center" maxW="100%">
              <AddressEntity
                address={{ hash: getCheckedSummedAddress(feePayer) }}
                isLoading={isLoading}
              />
              {executionSuccessBadge}
              {executionFailedBadge}
